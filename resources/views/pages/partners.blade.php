...
<section class="relative py-32 overflow-hidden">
    ...
    <div class="relative z-20 max-w-container mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h1 class="font-heading font-bold text-4xl md:text-5xl text-white mb-6">{{ $hero?->title ?? 'Our Partners & Sponsors' }}</h1>
        <p class="text-lg text-cream/90 max-w-2xl mx-auto">{{ $hero?->body ?? "Together with our valued partners, we are advancing Africa's coconut industry." }}</p>
    </div>
</section>
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 items-center">
                    ...
                    <div class="group bg-white rounded-card p-8 shadow-card hover:shadow-lg transition-all text-center">
                        ...
                        <div class="h-20 flex items-center justify-center mb-4">
                            <img src="{{ $partner->logo_url }}" alt="{{ $partner->organization_name }}" class="max-h-full grayscale group-hover:grayscale-0 transition-all">
                        </div>
                        @else
                        <div class="h-20 flex items-center justify-center mb-4">
